...
public class GridManager : MonoBehaviour
{
    ...
    public bool PlaceObject(Vector2Int gp, GameObject obj, bool blockWalking = true)
    {
        ...
        return RegisterOccupant(gp, obj, blockWalking);
    }

    /// <summary>
    /// すでに存在するオブジェクトをグリッドの占有物として登録する。
    /// CanPlaceチェックを行わずに強制的に登録します。
    /// </summary>
    public bool RegisterOccupant(Vector2Int gp, GameObject obj, bool blockWalking = true)
    {
        if (!IsValidPosition(gp)) return false;
