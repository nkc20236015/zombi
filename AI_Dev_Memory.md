...
*   **木の伐採演出 (`ResourceNode.cs`):**
    *   木（ResourceType.Wood）専用: 伐採中は DOPunchRotation で木がブルブル揺れる
    *   資源が0になると DOLocalRotate で前方（ローカルX軸）にバタンと倒れる
    *   倒れ始めと同時にColliderを無効化（NPCが引っかからない）
    *   倒れ終わった後、1秒待ってから非アクティブ化
    *   岩など他の資源は従来通りスケール縮小→非アクティブ化
...
*   木の倒れる方向はローカルX軸固定。不自然な場合はインスペクターの fallAngle を調整可能。
